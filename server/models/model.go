...
type User struct {
	...
	Password    string `gorm:"not null" json:"-" validate:"required,min=8"` // Hide password from JSON response
	IsAdmin     string `gorm:"default:'0'" json:"is_admin"`
	CookieToken string `json:"-"` // Hide cookie token from JSON response
	Posts       []Post
}
